...
### 1.2 项目背景
在快节奏的现代生活中，时间管理与习惯养成逐渐成为用户的刚性需求。本项目参考了主流市场上的"极简待办清单"类应用，旨在设计并实现一款界面清新、交互简单且功能实用的待办与习惯管理工具。该应用融合了番茄工作法、AI 智能对话助手、本地数据存储、广播通信、后台服务、位置服务及本地推荐算法，用以检验和展示在《移动互联网应用》课程中所掌握的 Android 开发技术。
...
|   Fragment (Habit, Todo, Pomodoro, Discover/Chat, Mine, Location)  |
...
*   **任务列表**：支持添加番茄钟任务，预设时长芯片（25/30/45/60 分钟）或自定义分钟数。
...
*   **自然语言操作**：用户可使用自然语言创建/删除/完成待办、习惯和番茄钟任务，AI 自动执行并反馈结果。
*   **API 配置**：首次使用提示配置 DeepSeek API Key，支持随时在"我的"→"AI 设置"中修改。

#### 4.4.2 关键技术点实现
*   **Retrofit API 集成**：`ChatApi`（POST `/chat/completions`，DeepSeek 兼容接口），`AiApi`（POST `/parse`，任务解析）。
*   **命令协议 (CMD Protocol)**：AI 回复中嵌入 `[CMD]{"action":"...",...}[/CMD]` JSON 块，由 `CommandParser` 提取并执行。支持 10 种操作：创建/删除/完成待办、创建/删除习惯、创建/删除番茄钟任务、列出待办/习惯/番茄钟。
*   **指令执行器**：`CommandParser.process()` 扫描 AI 回复中的所有命令块，提取 JSON 参数后通过 DAO 层操作数据库，返回执行日志。
*   **周一至周日解析**：`parseDays()` 支持数字串（`"124"`）与中文格式（`"周一、二、四"`）的星期解析，也支持从 `desc` 字段后备提取。
*   **对话管理**：消息持久化到 `chat_message` 表，按 `session_id` 分用户隔离。完整对话上下文（含 CMD 块与执行结果）随每次 API 请求发送。
*   **重试机制**：若 AI 回复不含命令但用户意图为操作请求，`retryCommand()` 自动重试一次并附加强提醒。
...
| 命令 | 参数 | 说明 |
|:---|:---|:---|
| `create_todo` | title, note, priority(1-3), due(yyyy-MM-dd HH:mm) | 创建待办 |
| `delete_todo` | title 或 id | 删除待办 |
| `complete_todo` | title 或 id | 完成待办 |
| `list_todos` | 无 | 列出所有待办 |
| `create_habit` | name, freq(每日/每周), days(如"124"), time(HH:mm), desc | 创建习惯 |
...
| 设置子项 | 对应功能描述 | 技术实现路径 |
|:---|:---|:---|
| **登录/注册** | 本地账号密码认证，支持记住密码和登录历史 | SHA-256 哈希存储密码；Android Keystore AES-256/GCM 加密记住的密码；SharedPreferences 存储登录历史（去重 LinkedHashSet） |
| **头像选择** | 40 款 emoji 头像可选 | GridView 网格选择器，SharedPreferences 按用户独立存储 |
| **AI 设置** | DeepSeek API Key 配置 | SharedPreferences 存储 API Key 和 URL；ChatClient 支持热切换 |
| **数据备份** | 本地数据导出与恢复 | `BackupManager`：利用 File Storage 将 SQLite 中的待办与习惯数据导出为本地 `.json` 备份文件（双存储：应用私有目录 + 公共 Downloads），支持从文件或 URI 恢复（追加式导入） |
| **密码锁** | 启动隐私安全保障 | SharedPreferences 存储 SHA-256 密码哈希值与开关状态。开启后每次冷启动弹出 `LockActivity` |
| **每日提醒** | 定时推送提醒打卡 | 结合 `AlarmManager` 与 `DailyReminderReceiver`，在用户设定时间发送本地通知 |
| **位置服务** | 获取当前位置与地址 | `LocationHelper`（FusedLocationProviderClient 单例）：支持单次定位（15s 超时 + 后备 last known）、LiveData 观察、反向地理编码（Geocoder）、历史记录（内存最多 50 条） |
...
*   **业务场景**：模拟外部桌面小部件（Widget）或其他辅助应用需要获取待办数据。
...
*   **技术实现**：DeepSeek Chat API 集成，通过系统提示注入 `[CMD]` 协议规范。`CommandParser` 本地解析并执行 AI 回复中的命令，通过 DAO 层操作 Room 数据库，执行结果内联反馈至对话历史。
...
| 位置记录 | 内存 (LiveData, 最多 50 条历史) | 运行时缓存 |
...
*   **自适应布局**：使用 `ConstraintLayout` 约束和 `RecyclerView`，确保在 16:9、20:9 等不同长宽比的安卓手机屏幕上，界面元素不发生重叠或变形。
...
*   **目标 SDK 版本**：API 36
*   **编译 SDK 版本**：API 36 (minorApiLevel 1)
